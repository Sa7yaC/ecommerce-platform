...
import { ArrowLeft, Trash2, AlertCircle, Plus, Layers, Footprints, Shirt, Check } from 'lucide-react';
...
export const ProductEdit: React.FC = () => {
  ...
  const displayCategories =
    storeCategories.length > 0 ? storeCategories : POPULAR_CATEGORIES;
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.sizes && product.sizes.length > 0) {
        ...
        if (
          ['footwear', 'shoe', 'shoes', 'sneaker', 'boot', 'sandal'].some((k) => catLower.includes(k)) ||
          product.sizes.some((s) => ['6', '7', '8', '9', '10', '11', '12'].includes(s.size))
        ) {
  ...
  const detectAndApplyCategoryPresets = (categoryName: string) => {
    ...
    if (['footwear', 'shoe', 'shoes', 'sneaker', 'boot', 'sandal'].some((k) => catLower.includes(k))) {
      setHasSizes(true);
      setSizeType('footwear');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Product and size inventory updated successfully.');
    } catch (err) {
    ...
    return (
      <div className="max-w-xl mx-auto my-16 p-8 bg-white rounded-3xl border border-[#E7E7E3] text-center space-y-4">
        ...
        <p className="text-xs text-[#6F6F6B]">
          Could not load the requested product for editing.
        </p>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#6F6F6B] mt-0.5">
            Modify details and size inventory for "{product.name}".
          </p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-[#F7F7F5] border border-[#E7E7E3] space-y-4">
          ...
          {hasSizes ? (
            <div className="space-y-4 pt-2">
              <p className="text-xs text-[#6F6F6B]">
                Specify available stock for each size. Total product stock will be automatically synced.
              </p>
              ...
              {sizes.length > 0 ? (
                <div className="space-y-2 pt-2">
                  ...
                  <div className="pt-2 flex justify-between items-center text-xs">
                    <span className="font-medium text-[#6F6F6B]">Total Combined Stock:</span>
                ...
                <p className="text-xs text-amber-700 italic">
                  Select at least one size variant above.
                </p>
              )}
            </div>
          ) : (
        ...
        <div className="space-y-4">
          <div className="p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
            ...
            <p className="text-xs text-red-700 leading-relaxed">
              Product <strong>"{product.name}"</strong> will be permanently removed.
            </p>
          </div>
